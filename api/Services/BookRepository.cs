...
namespace bookshelf.Services
{
    public class BookRepository : IBookRepository
    {
        ...
        {
            _books.Remove(id, out Book book);
            await Task.FromResult((object)null);
        }

        public async Task<int> Add(Book book)
        {
            book.Id = Interlocked.Increment(ref _seed);
            _books.TryAdd(book.Id, book);
            return await Task.FromResult(book.Id);
        }

        public async Task<IEnumerable<Book>> GetBooks(int page, int count)
        {
            return await Task.FromResult(_books.Values.Skip(page * count).Take(count));
        }

        public async Task<Book> GetById(int id)
        {
            return await Task.FromResult(_books.TryGetValue(id, out Book book) ? book : throw new BookNotFoundException());
        }

        public async Task Update(Book book)
        {
            _books.AddOrUpdate(book.Id, book, (id, old) => {
                return book;
            });
            await Task.FromResult((object)null);
        }
